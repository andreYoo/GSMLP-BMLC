...
def evaluate_all(distmat, query=None, gallery=None,
                 ...
                 query_cams=None, gallery_cams=None,wr=None,ep=None,
                 cmc_topk=(1, 5, 10, 20)):
    ...
    if wr!=None:
        wr.add_scalar("Eval/MeanAP", mAP,ep)
    print('CMC Scores')
    for k in cmc_topk:
        print('  top-{:<4}{:12.1%}'
              .format(k, all_cmc[k - 1]))
        if wr!=None:
            wr.add_scalar("CMC/Top-%d"%(k),all_cmc[k - 1],ep)
    return all_cmc[0]
...
class Evaluator(object):
    ...
    def evaluate(self, query_loader, gallery_loader, query, gallery, writter=None,epoch=None, output_feature=None):
        print('Extract feature from query images')
        query_features, _ = extract_features(self.model, query_loader, 1, output_feature)
        print('Extract feature from gallery images')
        gallery_features, _ = extract_features(self.model, gallery_loader, 1, output_feature)
        print('Computing distance between the two feature groups')
        distmat = pairwise_distance(query_features, gallery_features, query, gallery)
        print('Start to compute metric')
        return evaluate_all(distmat, query=query, gallery=gallery,wr=writter,ep=epoch)
